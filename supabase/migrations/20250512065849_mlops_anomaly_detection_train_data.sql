...
CREATE TABLE mlops.anomaly_detection_train_data (
    id             BIGSERIAL PRIMARY KEY,
    tenant_id      BIGINT REFERENCES tenants ON DELETE CASCADE NOT NULL,
    dataset_id     BIGINT REFERENCES mlops.anomaly_detection_datasets(id) ON DELETE CASCADE NOT NULL,
    name           TEXT NOT NULL,
...
COMMENT ON TABLE mlops.anomaly_detection_train_data IS '异常检测数据集样本表';

-- 创建触发器
DROP TRIGGER IF EXISTS update_anomaly_detection_train_data_timestamp ON mlops.anomaly_detection_train_data;
CREATE TRIGGER update_anomaly_detection_train_data_timestamp
    BEFORE UPDATE ON mlops.anomaly_detection_train_data
    FOR EACH ROW EXECUTE FUNCTION mlops.trigger_set_updated_at();

-- 启用RLS
ALTER TABLE IF EXISTS mlops.anomaly_detection_train_data ENABLE ROW LEVEL SECURITY;    
...
CREATE POLICY "Allow (scoped) read access on 'anomaly_detection_train_data' to users with permissions 'anomaly_detection_train_data.read'"
    ON mlops.anomaly_detection_train_data
    FOR SELECT
    USING (
