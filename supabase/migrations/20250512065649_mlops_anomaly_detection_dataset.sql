...
-- 创建表
CREATE TABLE anomaly_detection_datasets (
...
COMMENT ON TABLE anomaly_detection_datasets IS '异常检测数据集';

-- 创建触发器
DROP TRIGGER IF EXISTS update_anomaly_detection_datasets_timestamp ON anomaly_detection_datasets;
CREATE TRIGGER update_anomaly_detection_datasets_timestamp
    BEFORE UPDATE ON anomaly_detection_datasets
    FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at();

-- 启用RLS
ALTER TABLE IF EXISTS anomaly_detection_datasets ENABLE ROW LEVEL SECURITY;
